...
take advantage of it. See <<_messagesprovider, "Messages Provider" Angular Provider>> for how to configure the `prompt` and `confirm` functions.
Declared in `mc.core.messages` module inside <<_mc_util, "mc.util" Angular Package>>.

== Usage

NOTE: It is very convenient to use the <<_messages_panel, "Messages Panel" Angular Directive>> to render the current messages.
